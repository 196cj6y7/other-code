...
def RankPM2(_a, size):
    ...
    for str in strPM: # 選擇指定字元
        ...
        for i in range( len(_a)):
            ...
            for j in PM_str:# 在字串中尋找指定字元
                if j == str:     # 找到指定字元
                    temp1.append(_a[i][:]) #temp1 = [][]
                    temp1[i].append(str)
                    break
        #----------------------------------------
        if temp1 != []:
            ...
            for x in range(len(temp2)):
                # add equipment number ex.1,2,3,4
                temp2[x].append(x+1)
...
def check_list_print(ll, ii):
    # ex.  ll : [30, '興一機E/P敲擊器設備定期檢查', 'E', 1]
    num1 = ll[ii][2]
    num2 = ll[ii][3]
    dd = {'C':'SSC', '粉':'CA', '螺':'CL', 'E':'EP', '高':'L_HPWP'}
    urlc = 'E:\pm_dir' + dd[num1] + "'\'" + num2 + '.pdf' 
...
driver = webdriver.Edge(r"C:\Users\Administrator\Desktop\auto_mms\msedgedriver.exe",options=opts)
...
list2 = RankPM2(list1, size1) # ex. list2 = [30, '#2...','E', 2]
